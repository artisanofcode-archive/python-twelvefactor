...
$directory = "/home/vagrant/twelvefactor"
...
su -c "cd #{$directory} && #{$virtualenv}/bin/pip install --upgrade tox pytest hypothesis==3.4.2 sphinx wheel flake8" - vagrant
SCRIPT
...
Vagrant.configure("2") do |config|
  ...
  if command? 'watchman'
    config.trigger.after [:up, :resume], :stdout => true do
      ...
      run "watchman -- trigger #{$host_directory} docs 'docs/**/*.rst' 'docs/**/*.py' 'twelvefactor.py' -X 'docs/_build/**' -- vagrant exec 'make docs && echo'"
    end
